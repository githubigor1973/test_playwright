...
import { getGroupAppLinkForUser } from './utils/get-group-app-link';
...
test('test with dynamic Employer name and State', async ({ page }) => {
  ...
  await newPage.evaluate(() => {
    const start = document.querySelector('input[name="enrollmentStartDate"]');
    const end = document.querySelector('input[name="enrollmentEndDate"]');
    if (start) start.removeAttribute('readonly');
    if (end) end.removeAttribute('readonly');
  });
  ...
  await page.getByRole('option', { name: 'Date of eligibility loss' }).click();
