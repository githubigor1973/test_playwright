...
test('test with dynamic Employer name and State', async ({ page }) => {
  ...
  // ✅ Step 2: Login
  await page.goto(config.urlPPNationwide);
  await page.getByRole('textbox', { name: 'Enter your email' }).fill(config.userNamePPNationwide);
  ...
  await page.getByRole('textbox').fill('damian stdamnw1');
  await page.getByRole('textbox').press('Enter');
  //await page.locator('label').filter({ hasText: 'damian newstsym1227' }).click();
  await page.getByRole('button', { name: 'Next step' }).click();
  await page.getByRole('button', { name: 'Next step' }).click();
  await page.getByRole('textbox').fill('BR damiannw');
  await page.getByText('BR damiannw').click();
  await page.getByRole('textbox').press('Enter');
  await page.getByRole('button', { name: 'Next step' }).click();

  await page.getByRole('listbox').filter({ hasText: 'PR damiannw' }).click();
  await page.getByRole('option', { name: 'PR damiannw' }).click();
  await page.getByRole('button', { name: 'Next step' }).click();
  for (let i = 0; i < 10; i++) {
...
await page.waitForTimeout(6000);
  ...
  const proposalLink = page.getByRole('link', { name: /https:\/\/qa-nationwide-platform/ });
