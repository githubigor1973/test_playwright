...
test('📨 Send email & verify in Salesforce', async ({ page }) => {
  ...
  while (Date.now() - start < timeout) {
    ...
    const manualSubmissionNumber = process.env.MANUAL_SUBMISSION || ''; // Optional fallback

let row;

if (manualSubmissionNumber) {
  // ➕ Use manually provided Submission number
  console.log(`🔎 Using manual submission: ${manualSubmissionNumber}`);
  row = page.locator('tr', { hasText: manualSubmissionNumber });
  if (await row.first().isVisible()) {
    console.log('✅ Found row for manual submission');
    await row.first().click();
    found = true;
  } else {
    throw new Error(`❌ Could not find row with manual submission: ${manualSubmissionNumber}`);
  }

} else {
  // 🔁 Auto-mode: search by subject from sent email
  const timeout = 60000;
  ...
  while (Date.now() - start < timeout) {
    ...
    row = page.locator('tr', { hasText: subject });
    if (await row.first().isVisible()) {
      console.log('✅ Found the email row by subject');
      await row.first().click();
      found = true;
      break;
    }
  ...
  // ------------------- 🔍 Try manual fallback if email not detected automatically -------------------
if (!found) {
  const manualSubmission = 'SUB-45367'; // 📝 Replace this with the fallback submission ID
  console.log(`🟡 Using manual submission: ${manualSubmission}`);
  // 🔒 Safe locator — no strict mode
  const manualRow = page.locator('lightning-formatted-text', {
    hasText: manualSubmission,
  });
  if (await manualRow.first().isVisible()) {
    await manualRow.first().click();
    console.log('✅ Found row for manual submission');
  } else {
    throw new Error(`❌ Manual fallback submission "${manualSubmission}" not visible either.`);
  }
} else {
  // ✅ If found through polling logic, click as normal
  await page.locator('lightning-formatted-text', { hasText: subject }).first().click();
  }
}
} // <-- This closes the main "while" or "if" block
